...
type RoleDetails = {
  ...
  listing_job_boards: string | null;
  listing_social_media: string | null;
};
...
export default function EditRolePage() {
    ...
    const load = async () => {
      ...
      const [
        ...
        supabase
          .from("roles")
          .select(
            "id,title,client_id,contact_id,owner_user_id,target_date,status_code,industry,job_type,salary_text,work_experience,notes,address_line_1,address_line_2,city,county,postcode,address_text,is_remote,is_hybrid,number_of_positions,expected_revenue_per_position,total_expected_revenue,actual_revenue,job_description_html,requirements_html,benefits_html,listing_job_boards,listing_social_media",
          )
          .eq("id", roleId)
          .maybeSingle<RoleDetails>(),
        supabase
          .from("clients")
          .select(
      ...
      const assignedOwnerId = roleData.owner_user_id ?? user.id;
      ...
      setRoleStatuses((statusesData ?? []) as RoleStatus[]);
      setIndustryOptions(industries);

      setPostingTitle(roleData.title);
      ...
      setWorkExperience(roleData.work_experience ?? "none");
      setNotes(roleData.notes ?? "");
      setAddressLine1(roleData.address_line_1 ?? "");
      setAddressLine2(roleData.address_line_2 ?? "");
      setCity(roleData.city ?? "");
      setCounty(roleData.county ?? "");
      setPostcode(roleData.postcode ?? "");
      setAddressText(roleData.address_text ?? "");
      setIsRemote(Boolean(roleData.is_remote));
      setIsHybrid(Boolean(roleData.is_hybrid));
      setNumberOfPositions(
        typeof roleData.number_of_positions === "number"
          ? String(roleData.number_of_positions)
          : "",
      );
      setExpectedRevenuePerPosition(roleData.expected_revenue_per_position ?? "");
      setTotalExpectedRevenue(roleData.total_expected_revenue ?? "");
      setActualRevenue(roleData.actual_revenue ?? "");
      setJobDescriptionHtml(roleData.job_description_html ?? "");
      setRequirementsHtml(roleData.requirements_html ?? "");
      setBenefitsHtml(roleData.benefits_html ?? "");
      setListingJobBoards(roleData.listing_job_boards ?? "");
      setListingSocialMedia(roleData.listing_social_media ?? "");
      setIsLoading(false);
    };
  ...
  const handleDeleteRole = async () => {
    if (!roleId || !UUID_PATTERN.test(roleId)) {
      setDeleteError("Invalid role id.");
      return;
    }
  ...
  if (isLoading) {
    return (
      <main className={styles.loadingWrap}>
        <p className={styles.loadingText}>Loading role details...</p>
      </main>
    );
  }
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <Link href={`/admin/roles/${roleId ?? ""}`} className={styles.cancelLink}>
          Back to Profile
        </Link>
      </header>
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.actions}>
          ...
          <Link href={`/admin/roles/${roleId ?? ""}`} className={styles.cancelButton}>
            Cancel
          </Link>
          <button
            type="button"
        ...
        {showDeleteWarning ? (
          <div className={styles.deleteWarningCard}>
            <p className={styles.deleteWarningTitle}>Delete this role?</p>
            <p className={styles.deleteWarningText}>
              This permanently removes the role record and its linked applications.
            </p>
            {deleteError ? (
